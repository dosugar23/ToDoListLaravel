<div>
    ...
    {{-- First todo --}}
    <div class="flex mt-5 py-5 justify-between">
        <div>
            <input id="green-checkbox" wire:click='markCompleted({{  $todo->id }})' @if($todo->is_completed) checked
            @endif type="checkbox" class="w-4 h-4 text-green-600 bg-gray-100 rounded focus:ring-green-500
            dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600 ">
        </div>
        <div class="text-left flex-1 ml-5">
    
            @if ($edit == $todo->id)
            <x-text-input wire:model='editedTodo' class="w-full mr-2" />
            @else
            <span @if ($todo->is_completed ) class='line-through' @endif> {{ $todo->todo }}</span>
            @endif
        </div>
